...
public:
	...
	template<typename C>
	static int64_t count_symbols(C& str, const char a_symbol);
...
template<typename C>
int64_t StringUtils::count_symbols(C& str, const char a_symbol) {
	int64_t result = 0;
	for(int64_t base_id = 0; base_id < str.size(); ++base_id) {
		if(a_symbol == str[base_id]) {
			++result;
		}
	}
	return result;
